...
class MqttService {
  ...
  Future<bool> connect() async {
    ...
    try {
      ...
      final connMessage = MqttConnectMessage()
          .withClientIdentifier(AppConfig.mqttClientId)
          .authenticateAs(
              'mqtt_user', AppConfig.mqttPassword) // ✅ AQUÍ va user/pass
          .startClean()
          .withWillQos(MqttQos.atLeastOnce);
      ...
      print(
          '🔄 Connecting to MQTT broker at ${AppConfig.mqttBrokerUrl}:${AppConfig.mqttBrokerPort}...');
  ...
  void _handleMessage(MqttReceivedMessage<MqttMessage> message) {
    ...
    try {
      ...
      print(
          '📨 Received: $deviceId - $metric: ${telemetry.value}${telemetry.unit}');
    } catch (e) {
